...
SILVER_PATH      = "s3://omniroute-silver/poc-bootcamp-group1-silver/maintenance_schedules/"
...
def main():
    ...
    try:
        ...
        dyf = glue_context.create_dynamic_frame.from_options(
            connection_type="s3",
            connection_options={
                "paths": [f"s3://{BUCKET_NAME}/{BRONZE_PREFIX}"],
                "recurse": True
            },
            format="csv",
            format_options={"withHeader": True, "separator": ","},
            transformation_ctx="maintenance_schedules_source"
        )

        df = dyf.toDF()

        if df.isEmpty():
            log.info("No new maintenance_schedules files found by bookmark. Exiting.")
            job.commit()
            return

        log.info(f"Bronze data read successfully. Run date: {run_date} | Run year: {run_year}")
        ...
        log.error(f"Unexpected failure processing maintenance_schedules - {e}")
        raise
