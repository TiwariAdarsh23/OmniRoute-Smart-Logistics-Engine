...
BUCKET = "omniroute-bronze"
DATASET_BUCKET = "omniroute-silver"
...
def generate_out_of_domain_timestamps(logical_start):
    ...
    for i in range(-3, 4):
        if i:
            out_of_domain_timestamps.append(str(ts + i * day_ts))
...
def generate_txn_id(logical_date, i):
    clean_date = logical_date.strftime("%Y%m%d")
    return f"TXN_{clean_date}{i}"
...
def read_vehicle_registry(vehicle_type_map, year, month, date):
    path = (
        f"s3://{DATASET_BUCKET}/"
        f"{REGISTRY_PREFIX}"
...
def read_previous_odometer(year, month, date):
    logical_dt = datetime(year, month, date).date()
    prev_dt = logical_dt - timedelta(days=1)

    prev_path = (
        f"s3://{DATASET_BUCKET}/"
        f"poc-bootcamp-group1-silver/fuel_transactions/"
        f"year={prev_dt.year}/month={prev_dt.month}/date={prev_dt.day}/"
    )
    ...
    try:
        ...
        latest_odo = (
            df.dropna(subset=["vin", "odometer_reading"])
              .groupby("vin")["odometer_reading"]
              .max()
              .to_dict()
        )

        return defaultdict(float, latest_odo)

    except Exception as e:
        logger.warning(f"No previous fuel_transactions found: {e}")
        return defaultdict(float)
    
# Main dataset function

def generate_fuel_transactions_data(**context):
    ...
    vehicle_registry = read_vehicle_registry(vehicle_type_map, year, month, date)
    ...
    vin_odometer = read_previous_odometer(year, month, date)
    ...
    for _ in range(num_records):
        ...
        for _ in range(2):
            txn_id = bad_data(
                generate_txn_id(logical_dt, txn_counter),
                0.02,
                out_of_domain_txn
            )
